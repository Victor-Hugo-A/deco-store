...
export const metadata: Metadata = { title: "Confirmação de e-mail | DECO", robots: { index: false, follow: false }, referrer: "no-referrer" };
...
export default async function ConfirmationPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="grid min-h-screen place-items-center px-4 py-12">
    <section className="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-sm">
      <Link href="/" className="text-3xl font-black tracking-tight">DECO<span className="text-[#ff4d00]">.</span></Link>
      ...
      <Link href="/" className="mt-6 inline-block text-sm text-black/55">Voltar para as camisas</Link>
    </section>
  </main>;
}
